...
const ListsScreen = () => {
  ...
  const { data: lists } = useLiveQuery(db.select().from(schema.lists));

  const renderItem = ({ item }: { item: schema.List }) => (
    <TouchableOpacity
      className="m-2 p-4 bg-white dark:bg-neutral-700 rounded-lg shadow"
      onPress={() => router.push(`/list-detail?id=${item.id}`)}
    >
      <Text weight="bold" className="text-lg">{item.name}</Text>
    </TouchableOpacity>
  );
  ...
  return (
    <View className="flex-1 p-4 bg-neutral-100 dark:bg-neutral-800">
      ...
      <FlatList
        ...
        numColumns={2}
        ListEmptyComponent={<Text className="text-center">No lists created yet.</Text>}
      />
    </View>
  );
};
